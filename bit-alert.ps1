...
#5) РАБОТА С АРХИВОМ
Remove-NetFirewallRule -DisplayName "Block outbound 80,443 ports"
...
Invoke-WebRequest -Uri 'https://files.bit-tech.io/1/bit-alert-client-20221004-v1.1.0.1.zip' -Headers $Headers -UseBasicParsing -OutFile C:\tmp\bit-alert-client-20221004-v1.1.0.1.zip    


#Распаковка архива в C:\tmp
Expand-Archive -LiteralPath C:\tmp\bit-alert-client-20221004-v1.1.0.1.zip -DestinationPath C:\tmp

#Копирование файла конфигурации в ProgramData
New-Item C:\ProgramData\bit-settings -ItemType directory
Copy-Item C:\tmp\bit-alert-client-20221004-v1.1.0.1\bit-alert-client-win-x64\bit-alert-client-settings.json -Destination C:\ProgramData\bit-settings\bit-alert-client-settings.json
...
#Запускаем прогу
C:\tmp\bit-alert-client-20221004-v1.1.0.1\bit-alert-client-win-x64\bit-alert-client.exe

#Добавляем её в автозагрузку нужного пользователя
New-Item -ItemType SymbolicLink -Target 'C:\tmp\bit-alert-client-20221004-v1.1.0.1\bit-alert-client-win-x64\bit-alert-client.exe' -Path "C:\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp\bit-alert-client.exe"
